...
s首先请确保安装 [Node.js](https://nodejs.org/download/)
...
SHA256 算法通过给定的输入，将计算出一个唯一的哈希。相同的输入总会生成相同的哈希。
...
随机数不断迭代，知道哈希有效。在我们的例子中，有效的哈希值至少要四个 0 开头。寻找有效哈希对应的随机数的过程就称为开采（挖矿）。
...
随着难度值的提升，有效哈希的数量逐步减少，我们需要投入更多来找到一个有效哈希。
